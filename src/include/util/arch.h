 ...
 * memory barrier
 */
static inline void smp_cmb() { __asm__ __volatile__("" ::: "memory"); }
...
static inline void smp_wmb_tso() {
...
#define smp_prefetchr(__ptr) __builtin_prefetch((void*)__ptr, 0, 3)

#define smp_prefetchw(__ptr) __builtin_prefetch((void*)__ptr, 1, 3)
...
static inline uint64_t __attribute__((__always_inline__)) rdtsc_begin(void) {
...
static inline uint64_t __attribute__((__always_inline__)) rdtsc_end(void) {
  smp_cmb();
 ...
 * misc. functions
 */
static inline void cpu_relax() { __asm__ __volatile__("pause"); }

static inline void cpu_nop() { __asm__ __volatile__("nop"); }
...
static inline void ticketlock_init(struct ticketlock_t* lock) {
  lock->now_serving = lock->next_ticket = 0;
}

static inline void ticketlock_lock(struct ticketlock_t* lock) {
  int my_ticket = smp_faa(&lock->next_ticket, 1);
  while (lock->now_serving != my_ticket)
    ;
}

static inline void ticketlock_unlock(struct ticketlock_t* lock) {
  lock->now_serving++;
}
...
static inline unsigned int rand32_range(unsigned int* seed, unsigned int low,
                                        unsigned int high) {
  return low + (rand32(seed) % (high - low + 1));
}

static inline unsigned int rand32_seedless(void) {
