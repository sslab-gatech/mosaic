...
namespace core {
  ...
  public:
    ...
    const static size_t max_size_extension_fields_vertex_block =
        MAX_VERTICES_PER_TILE * sizeof(VectorType);

    constexpr static VertexType neutral_element = 0.;

    static VectorType* global_vector;
    ...
    sizeExtensionFieldsVertexBlock(const tile_stats_t& tile_stats) {
      return tile_stats.count_vertex_tgt * sizeof(VectorType);
    }

    static void fillExtensionFieldsVertexBlock(
      ...
      VectorType* vector = (VectorType*)extension_fields;
      for (int i = 0; i < edge_block_index->count_tgt_vertices; ++i) {
        vector[i] = SPMV::global_vector[tgt_index[i]];
      }
    }
    ...
    pullGather(const VertexType& u, VertexType& v, uint16_t id_src,
               ...
               void* extension_fields) {
      // Add up all the elements of the vector, the multiplicator is simply 1
      float vector_element = ((float*)extension_fields)[id_tgt];
      v = v + 1 * vector_element;
    }
    ...
    static void init_vertices(vertex_array_t<VertexType>* vertices,
      ...
      SPMV::global_vector =
          (VectorType*)malloc(sizeof(VectorType) * vertices->count);
      // initialize the vector with the index of every vertex
      for (int i = 0; i < vertices->count; ++i) {
        SPMV::global_vector[i] = i;
      }
    ...
    reset_vertices_tile_processor(VertexType* tgt_vertices,
      ...
      memset(tgt_vertices, 0, sizeof(VertexType) * response_vertices);
    }
  };
  VectorType* SPMV::global_vector = NULL;
}
}
