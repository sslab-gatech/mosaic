...
SG_NMIC = 1                     # The default number of edge-engines (on the Xeon Phi or Host) to use.
...
SG_NAPPLIER = 48        # Number of appliers, global,
SG_NGLOBALREDUCER = 8   # Number of global reducers.
...
SG_NFETCHER = 3         # Number of fetchers, instantiated PER edge-engine, local. Total number of reducers: SG_NMIC * SG_NFETCHER
SG_NINDEXREADER = 3     # Number of index readers PER edge-engine.
...
# Edge-engine config
SG_NREADER = 2     # When running on the host: Number of readers to instantiate to read the tiles into memory.
SG_NPROCESSOR = 3  # When running on the host: Number of tile processors to actually process the tiles with.
...
SG_RB_SIZE_PROCESSED = 1 * GB             # The size of the ringbuffer to save the result into.
...
SG_RB_SIZE_HOST_TILES = 1 * GB            # The size of the ringbuffer to receive the tile information from the host (fetcher) into.
...
# datapath
SG_DATAPATH_EDGE_ENGINE = {0: join(DATA_ROOT, "datapath", "nvme0")}
